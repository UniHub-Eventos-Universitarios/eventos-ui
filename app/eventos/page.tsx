...
function mapSortParam(sort?: string): Record<string, string> {
  switch (sort) {
    case "date-desc":
      return { sort: "data_inicio", order: "DESC" };
    case "title-asc":
      return { sort: "titulo", order: "ASC" };
    case "title-desc":
      return { sort: "titulo", order: "DESC" };
    case "registered-desc":
      return { sort: "data_inicio", order: "DESC" };
    default:
      return { sort: "data_inicio", order: "ASC" };
  }
}
...
export default async function EventosPage({
  ...
  const hasFilters =
    params.search || params.categoria || params.category || params.status;
              ...
              {events.length > 0 ? (
                ...
                <div className="flex flex-col items-center justify-center py-28 text-center">
                  ...
                  <h3 className="text-base font-semibold text-foreground mb-1">
                    {params.search
                      ? `Nenhum resultado para "${params.search}"`
                      : params.categoria ?? params.category
                      ? `Nenhum evento de ${params.categoria ?? params.category} encontrado`
                      : "Nenhum evento encontrado"}
                  </h3>
                  <p className="text-muted-foreground text-sm max-w-xs">
                    {params.categoria ?? params.category
                      ? `Com certeza teremos eventos de ${params.categoria ?? params.category} em breve. Fique de olho!`
                      : "Tente ajustar os filtros ou volte mais tarde para conferir os novos eventos."}
                  </p>
                </div>
              )}
            </div>
